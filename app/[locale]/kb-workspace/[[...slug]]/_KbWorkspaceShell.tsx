...
export type KbCmsContent = {
  ...
  SourceLabel?: string;
  CoreDefinition?: { html: string };
  DeductibleRules?: { html: string };
  Exceptions?: { html: string };
  StateDisclosure?: { html: string };
};
...
export default function KbWorkspaceShell({ cmsContent }: { cmsContent: KbCmsContent | null }) {
  ...
  const lob = cmsContent?.LineOfBusiness ?? 'Auto';
  const jurisdiction = cmsContent?.Jurisdiction ?? 'Florida';
  const sourceLabel = cmsContent?.SourceLabel ?? `${lob} Comprehensive (${jurisdiction} Override)`;
  const coreHtml = cmsContent?.CoreDefinition?.html ?? null;
  const deductibleHtml = cmsContent?.DeductibleRules?.html ?? null;
  const exceptionsHtml = cmsContent?.Exceptions?.html ?? null;
            ...
            <div className="bg-gray-100 rounded-lg p-4 mb-6">
              <div className="flex items-center gap-3 mb-2">
                ...
                <div>
                  ...
                  <p className="text-sm text-gray-500">Orlando, {jurisdiction}</p>
                </div>
              </div>
              <div className="mt-4 space-y-2">
                ...
                <div className="flex justify-between text-sm">
                  ...
                  <span className="font-semibold text-gray-800">{cmsContent?.PolicyTier ?? 'Platinum'}</span>
                </div>
                <div className="flex justify-between text-sm">
                  ...
                  <div className="p-4 space-y-3 font-mono text-xs">
                    <div className="fade-in flex gap-3">
                      ...
                      <div>
                        ...
                        <div className="text-gray-400 mt-0.5">Question + CRM context (State={jurisdiction}, Policy={lob}-{cmsContent?.PolicyTier ?? 'Platinum'}) sent to Opal Webhook</div>
                      </div>
                    </div>
                    {backstagePhase >= 2 && (
                      <div className="fade-in flex gap-3">
                        ...
                        <div>
                          ...
                          <div className="text-gray-400 mt-0.5">Webhook listener fires. Agent formulates GraphQL query for {jurisdiction} {cmsContent?.Topic ?? 'hail'} coverage rules in Optimizely Graph</div>
                        </div>
                      </div>
                    )}
                    {backstagePhase >= 3 && (
                      <div className="fade-in flex gap-3">
                        ...
                        <div>
                          ...
                          <div className="text-gray-400 mt-0.5">Returns {cmsContent?.VariationLabel ?? `${jurisdiction} variation`} — content item retrieved from CMS</div>
                        </div>
                      </div>
                    )}
                    {backstagePhase >= 4 && (
            ...
            <div ref={chatRef} className="flex-1 overflow-y-auto p-6 space-y-6 pb-12">
              ...
              <div className="flex gap-4">
                ...
                <div className="bg-white border border-gray-200 rounded-lg p-4 shadow-sm max-w-2xl">
                  <p className="text-gray-700">Hi Sarah, I see you&apos;re speaking with John Doe in {jurisdiction}. How can I assist you with his bundled policy today?</p>
                  {step === 0 && (
                    <div className="mt-4 space-y-2">
                      <button
                      ...
                      >
                        <span>&ldquo;Does his auto or home policy cover hail damage to his car parked in the driveway, and what is the {jurisdiction} deductible?&rdquo;</span>
              ...
              {step >= 1 && (
                <div className="flex gap-4 flex-row-reverse fade-in">
                  ...
                  <div className="bg-[#007BC7] text-white rounded-lg p-4 shadow-sm max-w-2xl">
                    <p>Does his auto or home policy cover hail damage to his car parked in the driveway, and what is the {jurisdiction} deductible?</p>
                  </div>
                </div>
              )}
              ...
              {step >= 3 && (
                <div className="flex gap-4 fade-in">
                  ...
                  <div className="bg-white border border-gray-200 rounded-lg p-5 shadow-sm max-w-2xl">
                    <h4 className="font-bold text-gray-900 mb-2">Coverage Found: {lob} Policy</h4>
                    {coreHtml ? (
                      <div
                    ...
                    <h4 className="font-bold text-gray-900 mb-2 mt-4">{jurisdiction} Deductible</h4>
                    {deductibleHtml ? (
                      <div
                        className="text-gray-700 text-sm mb-4 prose prose-sm max-w-none"
                        dangerouslySetInnerHTML={{ __html: deductibleHtml }}
                      />
                    ) : (
                      <p className="text-gray-700 text-sm mb-4">
                        Because the customer is in {jurisdiction}, the statutory deductible for hail damage on comprehensive claims is a <strong>$500 minimum</strong> unless a specific glass-only breakage occurs.
                      </p>
                    )}
              ...
              {/* Answer 2 — CMS-driven exceptions */}
              {step >= 7 && (
                <div className="flex gap-4 fade-in">
                  <OpalAvatar />
                  <div className="bg-white border border-gray-200 rounded-lg p-5 shadow-sm max-w-2xl border-l-4 border-l-[#007BC7]">
                    <h4 className="font-bold text-gray-900 mb-2">{jurisdiction} Windshield Exception Applied</h4>
                    {exceptionsHtml ? (
                      <div
                        className="text-gray-700 text-sm mb-4 prose prose-sm max-w-none"
                        dangerouslySetInnerHTML={{ __html: exceptionsHtml }}
                      />
                    ) : (
                      <>
                        <p className="text-gray-700 text-sm mb-4">
                  ...
                  <div className="bg-white border border-gray-200 rounded-lg p-5 shadow-sm max-w-2xl w-full">
                    ...
                    <div className="bg-gray-50 p-4 rounded border border-gray-200 mb-5 text-sm space-y-3">
                      <div className="flex justify-between border-b border-gray-200 pb-2">
                        ...
                        <span className="font-medium text-gray-900">John Doe (Orlando, {jurisdiction})</span>
                      </div>
                      <div className="flex justify-between border-b border-gray-200 pb-2">
                        <span className="text-gray-500">Policy:</span>
                        <span className="font-medium text-gray-900">{lob} - {cmsContent?.PolicyTier ?? 'Platinum'} Bundle</span>
                      </div>
                      <div className="flex justify-between border-b border-gray-200 pb-2">
                        <span className="text-gray-500">Incident type:</span>
                        <span className="font-medium text-gray-900">Comprehensive ({cmsContent?.Topic ?? 'Hail'})</span>
                      </div>
                      <div className="flex justify-between border-b border-gray-200 pb-2">
                      ...
                      <div className="flex justify-between pt-1">
                        ...
                        <span className="font-bold text-green-600 bg-green-100 px-2 py-0.5 rounded">$0.00 ({jurisdiction} Stat 627.7288)</span>
                      </div>
                    </div>
                    <button
