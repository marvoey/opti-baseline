/**
 * Create and publish standalone CMS content items (Paragraphs, ComplianceBlocks)
 * from a JSON array seed file.
 *
 * Unlike seed-experience.mjs, these are NOT Visual Builder experiences.
 * Properties are set directly on the content item — no composition PATCH needed.
 *
 * Supports two reference types in property values and the container field:
 *
 *   { "$file": "path/to/file.html" }
 *   Reads the file from disk relative to process.cwd() and inlines its content as a string.
 *
 *   { "$richTextFile": "seeds/library/discover-recommend/html/pack-01.html" }
 *   Same as $file but wraps the content as { value: { html: "..." } } for richText properties.
 *
 *   { "$env": "FOLDER_DISCOVER_RECOMMEND_PROSE_BLOCKS" }
 *   Reads the named environment variable at runtime.
 *
 * The input file may be a plain JSON array, or an envelope object:
 *   { "container": { "$env": "FOLDER_DISCOVER_RECOMMEND_PROSE_BLOCKS" }, "items": [...] }
 * The envelope's container becomes the default for every item in the array.
 *
 * Usage:
 *   node scripts/seed-paragraphs.mjs <path-to-items.json> [--dry-run] [--limit <n>]
 *   npm run seed:library:dr
 *
 * Flags:
 ...
 *  { "$richTextFile": "path" }  → { value: string } (richText property wrapper)
 *  { "$env": "VAR" }            → string (env var value)
 */
function resolveRef(value) {
  if (value && typeof value === 'object' && !Array.isArray(value)) {
    if (value['$richTextFile']) {
      const content = readFileSync(resolve(process.cwd(), value['$richTextFile']), 'utf8');
      return { value: { html: content } };
    }
    if (value['$file']) {
...
if (!filePath) {
  console.error('Usage: node scripts/seed-paragraphs.mjs <path-to-items.json> [--dry-run] [--limit <n>]');
  process.exit(1);
}
...
if (dryRun) {
  ...
  for (const item of items) {
    ...
    // Truncate long strings (e.g. HTML bodies) so the output is readable.
