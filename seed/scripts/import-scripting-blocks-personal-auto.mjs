// Import ../scripting-blocks-personal-auto.json into Optimizely CMS as prgv_ScriptingBlock items.
//
// Usage:
//   node seed/scripts/import-scripting-blocks-personal-auto.mjs [--dry-run] [--concurrency=N] --container=<key>
//
...
const DATA_FILE = join(SEED_DIR, '..', 'scripting-blocks-personal-auto.json');
...
async function importItem(item, index, total) {
  ...
  const body = {
    ...
    initialVersion: {
      ...
      properties: {
        VerbatimScript: { value: { html: properties.VerbatimScript } },
...
console.log(`Loaded ${items.length} item(s) from scripting-blocks-personal-auto.json`);
