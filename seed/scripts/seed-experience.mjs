 ...
 *
 * Usage:
 *   node seed/scripts/seed-experience.mjs <path-to-seed.json> [--live]
...
if (
  ...
  try {
    ...
    // No .env file — continue with whatever is already in process.env.
  }
}
...
const ROOT_CONTAINER_KEY =
  (live ? process.env.ROOT_CONTAINER_LIVE : process.env.ROOT_CONTAINER) ??
  '43f936c99b234ea397b261c538ad07c9';

function buildRouteSegment(seed) {
  // Allow explicit empty string for the homepage root URL.
  if (seed.routeSegment != null) return seed.routeSegment;
...
if (!filePath) {
  console.error('Usage: node seed/scripts/seed-experience.mjs <path-to-seed.json> [--live]');
  process.exit(1);
}
...
async function main() {
  ...
  while (true) {
    ...
    if (result.existed) {
      const url = routeSegment ? `/${routeSegment}` : '/';
    ...
    if (publish.routeConflict) {
      ...
      const url = routeSegment ? `/${routeSegment}` : '/';
      console.log(`\nURL "${url}" is already in use by a published page.`);
