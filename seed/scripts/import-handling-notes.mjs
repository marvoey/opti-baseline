// Import ../handling-notes-personal-auto.json into Optimizely CMS as prgv_HandlingNoteBlock items.
//
// Usage:
//   node seed/scripts/import-handling-notes.mjs [--dry-run] [--concurrency=N] --container=<key>
//
...
const DATA_FILE = join(SEED_DIR, '..', 'handling-notes-personal-auto.json');
...
async function importItem(item, index, total) {
  ...
  const body = {
    ...
    initialVersion: {
      ...
      properties: {
        NoteContent: { value: { html: properties.NoteContent } },
        ...
        ...(properties.SeverityLevel != null && { SeverityLevel: { value: properties.SeverityLevel } }),
        ...(properties.RuleCategory != null && { RuleCategory: { value: properties.RuleCategory } }),
...
console.log(`Loaded ${items.length} item(s) from handling-notes-personal-auto.json`);
