// Import ../standard-instructions-national.json into Optimizely CMS.
//
// Usage:
//   node seed/scripts/import-standard-instructions.mjs [--dry-run] [--concurrency=N] --container=<key>
//
...
const DATA_FILE = join(SEED_DIR, '..', 'standard-instructions-national.json');
...
async function importItem(item, index, total) {
  ...
  const body = {
    ...
    initialVersion: {
      ...
      properties: {
        InstructionText: { value: { html: properties.InstructionText } },
        ...(properties.TargetAudience != null && { TargetAudience: { value: properties.TargetAudience } }),
...
console.log(`Loaded ${items.length} item(s) from standard-instructions-national.json`);
