...
export function parseArgs() {
  ...
  return { dryRun, concurrency, container, limit };
}
...
// Recursively clones a composition node tree, assigning fresh UUIDs to every node id.
export function cloneComposition(node) {
  const { nodes, ...rest } = node;
  return {
    ...rest,
    id: uuidv4(),
    ...(nodes?.length ? { nodes: nodes.map(cloneComposition) } : {}),
  };
}
